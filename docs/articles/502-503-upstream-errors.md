---
layout: default
title: "502/503 and Origin Overload: When CDN Is Green but Origin Is Red"
category: Troubleshooting
description: "502 means gateway got bad response; 503 means temporarily unavailable. Users on CDN see 502—could be edge, but more often origin can't handle load—comparative …"
keywords: 502,503,origin overload,CDN origin,SpeedCE
...
502 means gateway got bad response; 503 means temporarily unavailable. Users on CDN see 502—could be edge, but more often origin can't handle load—comparative test decides.

In incidents like **502/503 and Origin Overload**, the most common ops mistake is **skipping steps**: changing code, swapping servers, or blaming carriers before confirming whether nodes nationwide are green or red. The correct order is always: multi-node impact check → comparative testing to narrow the layer (DNS/cert/CDN/origin/app) → targeted fix → retest until target met → archive. SpeedCE shines at step one and four: it gives you a real-time traffic map, not your desk-side gut feeling.
...
This is an **actionable long-form guide** on **502/503 and Origin Overload** (15–20 min read). Examples use free tool SpeedCE; the troubleshooting mindset applies to any multi-node probe.
...
|------|---------|---------------|
| 502 Bad Gateway | Invalid upstream response | Check PHP-FPM/Nginx upstream |
| 503 Service Unavailable | Overload or maintenance | Check pool/rate limit |
| Origin timeout | CDN waits too long | Increase timeout or scale |
| Health check | LB probe path | Probe URL must be green |
...
#### Scenario 1: User reports 502/503 and Origin Overload

**Symptoms**

User reports related to **502/503 and Origin Overload**: some provinces, carriers, or time windows fail while your local test looks fine. Common thread: single-point tests don't represent nationwide users—SpeedCE multi-node maps give objective samples.

For **502/503 and Origin Overload**, also log: **change timestamp**, **user province/carrier samples**, **persistent vs intermittent**. Combine with the map for faster root cause.
...
> **502/503 and Origin Overload** nationwide result: success rate **{XX%}**, anomalies in **{region}** (map attached). Likely cause **{XX}**, action **{XX}**, post-fix success **{XX%}**.

### Template 5: Forum / community "please review"

```
Subject: [Review] 502/503 and Origin Overload — SpeedCE three-carrier screenshots
