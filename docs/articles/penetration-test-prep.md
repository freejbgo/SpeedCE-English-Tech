---
layout: default
title: "Pre-Pentest External Attack Surface: Public Domain Speed Test Inventory"
category: Advanced
description: "Before pentest, inventory all public hosts with probes—find orphan domains attackers will too."
keywords: penetration test,attack surface,domain,SpeedCE,inventory
...
For **Pre-Pentest External Attack Surface**, this guide turns multi-node speed tests into a **repeatable process**. Same steps every incident or change—screenshot, compare, report.
...
This is an **actionable long-form guide** on **Pre-Pentest External Attack Surface** (15–20 min read). Examples use free tool SpeedCE; the troubleshooting mindset applies to any multi-node probe.
...
#### Scenario 1: User reports Pre-Pentest External Attack Surface

**Symptoms**

User reports related to **Pre-Pentest External Attack Surface**: some provinces, carriers, or time windows fail while your local test looks fine. Common thread: single-point tests don't represent nationwide users—SpeedCE multi-node maps give objective samples.

For **Pre-Pentest External Attack Surface**, also log: **change timestamp**, **user province/carrier samples**, **persistent vs intermittent**. Combine with the map for faster root cause.
...
> **Pre-Pentest External Attack Surface** nationwide result: success rate **{XX%}**, anomalies in **{region}** (map attached). Likely cause **{XX}**, action **{XX}**, post-fix success **{XX%}**.

### Template 5: Forum / community "please review"

```
Subject: [Review] Pre-Pentest External Attack Surface — SpeedCE three-carrier screenshots
