---
layout: default
title: "Load Balancer and Health Checks: Half Green Half Red Architecture Patterns"
category: Troubleshooting
description: "Multiple backends, one down—DNS round robin or bad LB config feels like \"sometimes works.\" Multi-node probes + repeated tests confirm."
keywords: load balancer,health check,high availability,SpeedCE,operations
...
This is an **actionable long-form guide** on **Load Balancer and Health Checks** (15–20 min read). Examples use free tool SpeedCE; the troubleshooting mindset applies to any multi-node probe.
...
|------|---------|---------------|
| Health check | Probe path | /health must return 200 |
| Weight | Traffic split | Bad node should drop |
| Session stickiness | Sticky sessions | Hit bad node worse |
| DNS round robin | Multiple A records | One bad IP → sporadic red |
...
#### Scenario 1: User reports Load Balancer and Health Checks

**Symptoms**

User reports related to **Load Balancer and Health Checks**: some provinces, carriers, or time windows fail while your local test looks fine. Common thread: single-point tests don't represent nationwide users—SpeedCE multi-node maps give objective samples.

For **Load Balancer and Health Checks**, also log: **change timestamp**, **user province/carrier samples**, **persistent vs intermittent**. Combine with the map for faster root cause.
...
> **Load Balancer and Health Checks** nationwide result: success rate **{XX%}**, anomalies in **{region}** (map attached). Likely cause **{XX}**, action **{XX}**, post-fix success **{XX%}**.

### Template 5: Forum / community "please review"

```
Subject: [Review] Load Balancer and Health Checks — SpeedCE three-carrier screenshots
