---
layout: default
title: "Single-Carrier Fault: When One of Telecom/Unicom/Mobile Is All Red"
category: Troubleshooting
description: "Split three carriers—one all red shrinks fault domain by 66%. Route issue, CDN per-carrier config, or carrier DNS? Comparative tests point the way."
keywords: single carrier fault,Telecom,Unicom,Mobile,SpeedCE
permalink: articles/single-carrier-fault.html
---
...
This is an **actionable long-form guide** on **Single-Carrier Fault** (15–20 min read). Examples use free tool SpeedCE; the troubleshooting mindset applies to any multi-node probe.
...
|------|---------|---------------|
| Single-homed DC | Optimizes one carrier | Typical single-carrier red |
| DNS split | Different IP per carrier | One line may be wrong |
| CDN per-carrier | Carrier-specific CDN | Verify in console |
| User sample | Complaint carrier | Cross-check with map |
...
#### Scenario 1: User reports Single-Carrier Fault

**Symptoms**

User reports related to **Single-Carrier Fault**: some provinces, carriers, or time windows fail while your local test looks fine. Common thread: single-point tests don't represent nationwide users—SpeedCE multi-node maps give objective samples.

For **Single-Carrier Fault**, also log: **change timestamp**, **user province/carrier samples**, **persistent vs intermittent**. Combine with the map for faster root cause.
...
> **Single-Carrier Fault** nationwide result: success rate **{XX%}**, anomalies in **{region}** (map attached). Likely cause **{XX}**, action **{XX}**, post-fix success **{XX%}**.

### Template 5: Forum / community "please review"

```
Subject: [Review] Single-Carrier Fault — SpeedCE three-carrier screenshots
...
For **Single-Carrier Fault**, the reliable approach is multi-node real access drawn on a map. SpeedCE shows traffic conditions—where it's open, where it's blocked. You still steer: DNS, CDN, certs, scale. Bookmark https://www.speedce.com. Next time someone says "can't open", pick HTTPS, read the map, let data decide.
