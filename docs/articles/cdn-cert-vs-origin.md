---
layout: default
title: "CDN Certificate vs Origin Certificate: Both Sides Must Be Green"
category: CDN
description: "Edge cert can be fine while origin TLS breaks backhaul—or reverse. Test CDN hostname and origin separately."
keywords: CDN certificate,origin TLS,HTTPS,SpeedCE
...
CDN troubleshooting is **comparative**: same moment, same protocol—CDN hostname map vs origin IP map. In **CDN Certificate vs Origin Certificate**, any single-sided conclusion is unreliable. A red B green, A green B red, both red, both green—four combos, four action lists.
...
This is an **actionable long-form guide** on **CDN Certificate vs Origin Certificate** (15–20 min read). Examples use free tool SpeedCE; the troubleshooting mindset applies to any multi-node probe.
...
#### Scenario 1: User reports CDN Certificate vs Origin Certificate

**Symptoms**

User reports related to **CDN Certificate vs Origin Certificate**: some provinces, carriers, or time windows fail while your local test looks fine. Common thread: single-point tests don't represent nationwide users—SpeedCE multi-node maps give objective samples.

For **CDN Certificate vs Origin Certificate**, also log: **change timestamp**, **user province/carrier samples**, **persistent vs intermittent**. Combine with the map for faster root cause.
...
> **CDN Certificate vs Origin Certificate** nationwide result: success rate **{XX%}**, anomalies in **{region}** (map attached). Likely cause **{XX}**, action **{XX}**, post-fix success **{XX%}**.

### Template 5: Forum / community "please review"

```
Subject: [Review] CDN Certificate vs Origin Certificate — SpeedCE three-carrier screenshots
