---
layout: default
title: "Edge Functions/Workers Failures: Main Domain Green but Rules Not Applied"
category: CDN
description: "Workers and edge rules fail independently of origin. Test both default route and rule-triggered paths."
keywords: edge functions,Workers,CDN,SpeedCE,troubleshooting
...
CDN troubleshooting is **comparative**: same moment, same protocol—CDN hostname map vs origin IP map. In **Edge Functions/Workers Failures**, any single-sided conclusion is unreliable. A red B green, A green B red, both red, both green—four combos, four action lists.
...
This is an **actionable long-form guide** on **Edge Functions/Workers Failures** (15–20 min read). Examples use free tool SpeedCE; the troubleshooting mindset applies to any multi-node probe.
...
#### Scenario 1: User reports Edge Functions/Workers Failures

**Symptoms**

User reports related to **Edge Functions/Workers Failures**: some provinces, carriers, or time windows fail while your local test looks fine. Common thread: single-point tests don't represent nationwide users—SpeedCE multi-node maps give objective samples.

For **Edge Functions/Workers Failures**, also log: **change timestamp**, **user province/carrier samples**, **persistent vs intermittent**. Combine with the map for faster root cause.
...
> **Edge Functions/Workers Failures** nationwide result: success rate **{XX%}**, anomalies in **{region}** (map attached). Likely cause **{XX}**, action **{XX}**, post-fix success **{XX%}**.

### Template 5: Forum / community "please review"

```
Subject: [Review] Edge Functions/Workers Failures — SpeedCE three-carrier screenshots
