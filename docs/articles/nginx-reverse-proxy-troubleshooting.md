---
layout: default
title: "Nginx Reverse Proxy Troubleshooting: 8 Typical Config Errors When Main Site Is Green but API Is Red"
category: Troubleshooting
description: "Nginx fronts countless sites. One wrong server_name or missing proxy_pass means homepage works, APIs die. Postman works locally; nationwide users fail."
keywords: Nginx,reverse proxy,502,subdomain,SpeedCE
...
Nginx fronts countless sites. One wrong server_name or missing proxy_pass means homepage works, APIs die. Postman works locally; nationwide users fail.

In incidents like **Nginx Reverse Proxy Troubleshooting**, the most common ops mistake is **skipping steps**: changing code, swapping servers, or blaming carriers before confirming whether nodes nationwide are green or red. The correct order is always: multi-node impact check → comparative testing to narrow the layer (DNS/cert/CDN/origin/app) → targeted fix → retest until target met → archive. SpeedCE shines at step one and four: it gives you a real-time traffic map, not your desk-side gut feeling.
...
This is an **actionable long-form guide** on **Nginx Reverse Proxy Troubleshooting** (15–20 min read). Examples use free tool SpeedCE; the troubleshooting mindset applies to any multi-node probe.
...
|------|---------|---------------|
| server_name | Matched hostnames | Separate server block per public domain |
| proxy_pass | Upstream URL | Trailing slash affects URI |
| upstream | Backend pool | One dead backend can drag all |
| ssl_certificate | Cert path | Every HTTPS server block needs pair |
...
#### Scenario 1: User reports Nginx Reverse Proxy Troubleshooting

**Symptoms**

User reports related to **Nginx Reverse Proxy Troubleshooting**: some provinces, carriers, or time windows fail while your local test looks fine. Common thread: single-point tests don't represent nationwide users—SpeedCE multi-node maps give objective samples.

For **Nginx Reverse Proxy Troubleshooting**, also log: **change timestamp**, **user province/carrier samples**, **persistent vs intermittent**. Combine with the map for faster root cause.
...
> **Nginx Reverse Proxy Troubleshooting** nationwide result: success rate **{XX%}**, anomalies in **{region}** (map attached). Likely cause **{XX}**, action **{XX}**, post-fix success **{XX%}**.

### Template 5: Forum / community "please review"

```
Subject: [Review] Nginx Reverse Proxy Troubleshooting — SpeedCE three-carrier screenshots
